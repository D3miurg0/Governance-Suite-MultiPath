...
class MigrationModule:
    ...
    def execute_migration(
        ...
        permissions: bool, parallel_mode: bool,
        flat_copy_mode: bool, overwrite: bool = False
    ):
        self.copied_count = self.omitted_count = self.error_count = self.duplicate_count = 0
        self.duplicate_list = []
        ...
        self.core.log_audit("MIGRATION", {
            ...
            "copied": self.copied_count, "omitted": self.omitted_count,
            "errors": self.error_count, "duplicates": self.duplicate_count,
            "elapsed": f"{elapsed:.2f}s"
        })
        print(f"\n--- Resumen: Copiados: {self.copied_count} | Omitidos: {self.omitted_count} | Errores: {self.error_count} | Duplicados: {self.duplicate_count} ---")
    ...
    def _parallel_migration(self, src, dst, start_date, end_date, permissions, flat_copy_mode, overwrite):
        ...
        for src_f, dst_f in tqdm(sorted(folders_to_create.items(), key=lambda x: len(x[1])), desc="Carpetas"):
            try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=Config.MAX_WORKERS) as executor:
            futures = {executor.submit(self._copy_file_worker, s, d, permissions): s for s, d in files_to_copy}
            for fut in tqdm(concurrent.futures.as_completed(futures), total=len(files_to_copy), desc="Copiando"):
                if "COPIADO" in fut.result():
                    with self.core.global_lock:
                        self.copied_count += 1
