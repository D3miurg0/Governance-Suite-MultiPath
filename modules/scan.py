...
from core.compat import tqdm, USE_TQDM
...
class ScanModule:
    ...
    def _get_file_metadata(self, stats: os.stat_result):
        try:
            ...
            year_str = (
                str(year_num)
                if Config.MIN_YEAR_VALID <= year_num <= Config.MAX_YEAR_VALID
                else "Fecha_Invalida"
            )
            return (
                dt_mod.strftime("%Y-%m-%d %H:%M:%S"),
                dt_cre.strftime("%Y-%m-%d %H:%M:%S"),
                year_str,
            )
        except Exception:
    ...
    def _close_handles(self, file_handles: dict):
        """Cierra todos los CSV abiertos de forma segura."""
    ...
    def run_scan(self, drive_path: str):
        ...
        try:
                ...
                try:
                    for root, dirs, files in os.walk(long_path, onerror=self.core.on_walk_error):
                        for name in files:
                            ...
                            futures_set.add(
                                executor.submit(self._worker_process_file_scan, full_path, name)
                            )
                            # Drenar el buffer para evitar acumulación de memoria
            ...
            # Garantizado: siempre se cierran los CSV, incluso ante excepción
            self._close_handles(file_handles)
            if pbar:
                pbar.close()
